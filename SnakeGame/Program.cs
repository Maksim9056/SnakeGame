...
namespace SnakeGame
{
    ...
    //        private const char CharacterSymbol = 'X';
...
//        // Проверка соударения змеи со стена�
...
//                // Проверка, чтобы пища не появилась на змее или персонаже
...
//                case ConsoleKey.D:
//                    if (directionX != -1) // Запретить поворот на 180 градусво
//                    {
//                        directionX = 1;
//                        directionY = 0;
//                    }
...
//В этом примере создана простая консольная игра "Змейка и персонаж". Змея может двигаться по полю, поедая пищу и растягиваясь.Главная цель игрока - избегать коллизий со стенами и змеей и убегать от персонажа.
//Управление змейкой осуществляется с помощью стрелок на клавиатуре. При коллизии со стеной, собственным телом или персонажем игра завершается.

//Надеюсь, этот пример поможет тебе начать создание своей змейки и персонажа! Если у тебя возникнут еще вопросы или требуется дополнительная помощь, не стесняйся спрашивать.
